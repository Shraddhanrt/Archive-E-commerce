...
class ShippingCostCalculate extends Model
{
    ...
    public static function getEasyParsals($send_code, $send_state, $send_country, $cartcode){
        ...
        $total_weight = Cart::where('code', $cartcode)->sum('weight');
		...
		$action = "EPRateCheckingBulk";
           ...
           $data ='';
            foreach($array1->result[0]->rates as $item){
				if($country->code == 'MY'){
					if($item->courier_id == 'EP-CS0UO' || $item->courier_id == 'EP-CR0DP'){
						$data .= "
						
						<option value='".$item->service_id."?".$item->shipment_price."?".$item->courier_name."?".$item->delivery."?".$item->courier_logo."?".$pointid."?".$country->code."'>Free shipping via ".$item->service_name."</option>";
						 }
				}
				elseif($country->code =='SG'){
					if($item->courier_id == 'EP-CR0AL'){
						...
						<option value='".$item->service_id."?".$item->shipment_price."?".$item->courier_name."?".$item->delivery."?".$item->courier_logo."?".$pointid."?".$country->code."'>".$item->service_name."@MYR".$item->shipment_price."</option>";
